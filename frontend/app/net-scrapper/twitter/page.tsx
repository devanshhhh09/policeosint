...
export default function TwitterPage() {
  ...
  const load = async () => {
    try {
      const [s, c, i, st] = await Promise.all([
        api.get('/scrapper/sources?platform=twitter'),
        api.get('/scrapper/content?platform=twitter&per_page=30'),
        api.get('/scrapper/indicators?platform=twitter&per_page=20'),
  ...
  const analyzeText = async (text: string) => {
    const res = await api.post('/scrapper/analyze', { text, platform: 'twitter' })
    const d   = res.data
    alert(`Category: ${d.category}\nRisk: ${d.risk_score}/100\nIOCs: ${d.indicator_count}`)
  }
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center gap-3 mb-5">
        <div className="w-10 h-10 rounded-xl bg-sky-950 flex items-center justify-center">
          <i className="ti ti-brand-x text-sky-400 text-xl" />
        </div>
        <div>
          <h1 className="page-title">X (Twitter) Intelligence Dashboard</h1>
          <p className="text-gray-500 text-sm">
            Scam trends · Fake job postings · Investment fraud · Crypto scam profiles
          </p>
        </div>
        <div className="ml-auto flex gap-2">
          {stats && (
            <>
              ...
              <div className="card px-3 py-2 text-center">
                <div className="text-lg font-bold text-sky-400">{stats.total_sources || 0}</div>
      ...
      {/* Add source */}
      <div className="card p-4 mb-5 border-sky-900">
        <div className="flex gap-3">
          <div className="relative flex-1">
            <i className="ti ti-brand-x absolute left-3 top-1/2 -translate-y-1/2 text-sky-400" />
            <input value={identifier} onChange={e => setIdentifier(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && addSource()}
              placeholder="@username  or  https://twitter.com/user  or  keyword to track"
        ...
        <p className="text-xs text-gray-600 mt-2">
          ...
          Add Twitter/X API credentials to .env for live monitoring. Demo mode active without credentials.
        </p>
      </div>
        ...
        <div className="space-y-3">
          {content.length === 0 ? (
            <div className="card p-10 text-center text-gray-500">
              <i className="ti ti-brand-x text-4xl block mb-3 opacity-30" />
              <p className="font-medium text-white mb-1">No posts yet</p>
              <p className="text-sm">Add an account or keyword above and click Scrape</p>
            </div>
          ) : content.map((c: any) => (
            <div key={c.id} className={`card p-4 border ${
              ...
              <div className="flex items-start gap-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                  ...
                  <i className="ti ti-brand-x text-sm text-sky-400" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {c.author && (
                      <span className="text-xs text-sky-400 font-mono">@{c.author}</span>
                    )}
        ...
        <div className="card overflow-hidden">
          {sources.length === 0 ? (
            <div className="p-8 text-center text-gray-500 text-sm">
              No accounts tracked yet. Add one above.
            </div>
          ) : sources.map((s: any, i: number) => (
            <div key={s.id}
              className={`flex items-center gap-3 px-4 py-3
                          ${i < sources.length-1 ? 'border-b border-gray-800' : ''}`}>
              <div className="w-8 h-8 rounded-full bg-sky-950 flex items-center justify-center flex-shrink-0">
                <i className="ti ti-brand-x text-sky-400 text-sm" />
              </div>
              <div className="flex-1 min-w-0">
