...
export default function DomainPage() {
  ...
  const run = async () => {
    ...
    try {
      const res = await investigateAPI.start({ investigation_type: 'domain', query: query.trim(), query_type: 'domain' })
    ...
    <div className="p-6 max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 rounded-xl bg-teal-950 flex items-center justify-center">
          <i className="ti ti-world text-teal-400 text-xl" />
        </div>
        <div>
          <h1 className="page-title">Domain & Website Investigation</h1>
          <p className="text-gray-500 text-sm">WHOIS · DNS · SSL · Subdomains · VirusTotal</p>
        </div>
      </div>
      ...
      {loading && (
        <div className="card p-8 text-center space-y-3">
          ...
          <p className="text-gray-400 text-sm">Running WHOIS · DNS · SSL · crt.sh · VirusTotal…</p>
          <p className="text-gray-600 text-xs">Domain investigations take 5–10 seconds</p>
        </div>
      )}
      ...
      {result && !loading && (
        <>
          <div className="card p-4 mb-4 flex items-center gap-4">
            ...
            <div className="text-xs text-gray-600 max-w-48 truncate">{result.summary}</div>
          </div>

          {result.results?.map((r: any, i: number) => (
            <div key={i} className="card overflow-hidden mb-4">
              <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-800">
                <i className="ti ti-world text-teal-400 text-lg" />
              ...
              <table className="w-full text-sm">
                <tbody>
                  {Object.entries(r.parsed_data || {})
                    .filter(([k]) => !['name','severity'].includes(k))
                    .map(([k, v], j) => (
                      <tr key={j} className="border-b border-gray-800/50 last:border-0">
                        ...
                        <td className={`px-4 py-2.5 font-medium align-top ${
                          String(v).includes('⚠')||k==='is_new_domain'&&v===true ? 'text-amber-400' :
